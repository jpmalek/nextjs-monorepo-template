...
This is a production-ready monorepo setup using Turborepo, Next.js, NestJS, Changesets, Vercel, and GitHub Actions for CI/CD. Each app in apps/ and each package in packages/ is independently versioned and deployed.
...
- npnm version is handled by CI and tracks with package.json. To update pnpm to the latest version, run `corepack use pnpm@latest`
...
- **API Service**: NestJS API server running on port 3001
...
This Docker environment is designed to closely match the Vercel production environment
...
### 14. Use Docker for local development
