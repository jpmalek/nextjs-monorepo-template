...
## Dev Dependencies
- `@changesets/changelog-github` (^0.5.2): A changelog entry generator for GitHub that links to commits, PRs and users
- `@changesets/cli` (^2.29.8): Organise your package versioning and publishing to make both contributors and maintainers happy
...
- `@eslint/js` (^9.39.1): ESLint JavaScript language implementation
- `@next/eslint-plugin-next` (15.5.3): ESLint plugin for Next.js.
- `@tailwindcss/postcss` (^4.1.18): PostCSS plugin for Tailwind CSS, a utility-first CSS framework for rapidly building custom user interfaces
...
- `@types/node` (^24.10.3): TypeScript definitions for node
...
- `@typescript-eslint/eslint-plugin` (^8.49.0): TypeScript plugin for ESLint
- `@typescript-eslint/parser` (^8.49.0): An ESLint custom parser which leverages TypeScript ESTree
- `@vitejs/plugin-react` (^5.1.2): The default Vite plugin for React projects
- `clsx` (^2.1.1): A tiny (239B) utility for constructing className strings conditionally.
- `cross-env` (^10.1.0): Run scripts that set and use environment variables across platforms
- `eslint` (^9.39.1): An AST-based pattern checker for JavaScript.
- `eslint-plugin-next` (^0.0.0): No description available.
- `execa` (^9.6.1): Process execution for humans
...
- `next` (15.5.3): The React Framework
- `node-fetch` (^3.3.2): A light-weight module that brings Fetch API to node.js
- `pnpm` (^10.25.0): Fast, disk space efficient package manager
- `react` (^19.2.3): React is a JavaScript library for building user interfaces.
- `react-dom` (^19.2.3): React package for working with the DOM.
- `tailwindcss` (^4.1.18): A utility-first CSS framework for rapidly building custom user interfaces.
- `turbo` (^2.6.3): Turborepo is a high-performance build system for JavaScript and TypeScript codebases.
...
- `webpack` (^5.103.0): Packs ECMAScript/CommonJs/AMD modules for the browser. Allows you to split your codebase into multiple bundles, which can be loaded on demand. Supports loaders to preprocess files, i.e. json, jsx, es7, css, less, ... and your custom stuff.